...
title: "Brain tools"
sidebarTitle: "Brain"
description: "Twenty-three tools over roles, brain dumps, highlights, notes, standing rules and the four supporting collections."
icon: "brain"
---
...
_Append to a role's brain dump_
...
| Argument | Type | |
| --- | --- | --- |
| `dryRun` | boolean | Report what would happen and write nothing. Default false — pass true to preview. |
| `source` | string | What the document was, e.g. 'resume' or 'LinkedIn export'. Titles the saved note. |
| `sourceText` | string | The document as it arrived, in full. Filed as one note so the detail a summary drops stays searchable. |
| `profile` | object |  |
| `roles` | object[] | One per job, newest first or oldest first — order does not matter. |
| `education` | object[] | Schools, degrees, dates. |
| `projects` | object[] | Side projects, open source, anything with its own name. |
| `skills` | object[] | Grouped as the document groups them. One group called 'Skills' is fine when it does not group them. |
| `certifications` | object[] | Certificates and licences. |
