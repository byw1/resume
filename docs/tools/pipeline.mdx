...
description: "Twenty-four tools over applications, stages, the activity timeline, tasks, follow-ups, saved views, sharing and diagnosis."
icon: "list-check"
---
...
_List the source channels on file_
...
| `sources` | string[] | Where it came from, and several at once is normal: ['LinkedIn', 'Referral'] for a posting a friend also flagged. Free strings; call list_application_sources first and reuse the person's existing spellings where they fit. |
...
| `sources` | string[] | The full list of where it came from — replaces what is there |
