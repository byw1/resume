...
| `PUBLIC_URL` | Where this instance is reachable, for invitation links and the sign-in URL. On Railway, `RAILWAY_PUBLIC_DOMAIN` is picked up automatically. |
| `PDF_CHROMIUM_PATH` | Path to a Chromium for server-side PDF rendering. Only needed if it lives somewhere unusual. |
| `PORT` | Docker Compose only — the host port. The container stays on 3000. |
