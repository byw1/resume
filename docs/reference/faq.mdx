...
<AccordionGroup>
  ...
  <Accordion title="Can I import my existing resume or LinkedIn export?">
    ...
    says — a role per job with its bullets, education, skills, certifications, and the
    raw text kept as a note so nothing a summary drops is lost. Ask for a dry run first
    if you want to see what will land before it lands.

    It adds rather than overwrites. A job already on file is left exactly as it is, a
    profile field you have filled in is kept, and skill lists merge — so importing the
    same document twice writes nothing the second time.

    If you have not connected an assistant yet, the brain page has an **Import** button
    that takes pasted text and shows you what it read before saving anything. It guesses
    at headings, so check it; the conversation does this properly.
