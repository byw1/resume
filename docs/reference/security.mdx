...
Only the owner may create admins. An admin can act on members only — which is what stops
the admin area being a route to taking over an instance.
...
Your own [standing rules](/concepts/brain#standing-rules) are prepended to that briefing,
so they constrain the work before any tool is called.
