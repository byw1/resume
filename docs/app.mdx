...
The left rail holds Dashboard, Brain, Resumes, CRM and Pipeline. Docs, Settings and — for
admins — Admin live in the profile menu at the bottom. `⌘K` / `Ctrl+K` opens a search
palette: somewhere to go, something to create, and every role, resume and application by
name.

Both themes are supported; the toggle is top right. **Everything autosaves** — there is no
save button anywhere, and a small indicator tells you when a change has landed.
