...
client would and tells you how many tools answered — 80 for a member, 108 for an admin.
...
account: what the four areas are, which tool to reach for first in each, that the
connection URL is a credential, and the standing rules — never invent experience,
employers, dates or metrics; use `append_role_brain_dump` rather than `update_role` for
new material; `update_resume` and `update_role` replace what you send, so read first and
write back whole.

Any note you have saved with `kind: "GUARDRAIL"` is prepended to that briefing, so your
own standing rules constrain the work before a single tool is called. See
