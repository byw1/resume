...
everything that carried it and leaves those records otherwise untouched — unlike
`delete_company`, which refuses. A label you cannot remove is worse than one you delete by
mistake.
...
<CardGroup cols={3}>
  ...
  <Card title="list_tasks" icon="square-check">
    ...
    `update_task` moves one, `complete_task` ticks it off, `delete_task` removes it.
  </Card>
</CardGroup>
...
the **Filter** button beside the search: its tags, the company, which resume
...
living in the URL, because a saved view is a cut of the data and this is not. The company,
the role title and the stage are never in the list: a card without them is not shorter, it
is unreadable, and in the table the stage cell is the editor the table exists for.

**Tasks** is the page you clear the list from, and the dashboard's five-line version links
to it. Two columns, deliberately not merged: the things you wrote down, grouped by overdue,
