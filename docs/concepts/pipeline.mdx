...
the count of it on every other screen. **Analytics** is where the numbers went: the funnel
drawn as a flow chart, the diagnosis, the response rate, and a **Share chart** button that
hands you the flow chart as a PNG or an SVG with no company or role name anywhere on it.
**Pipeline** is the board, table and calendar. Opening an application slides it in from the right, so you keep your
...
used to sit there and now live on Analytics, because you open this app to do the next
thing rather than to read your own statistics. Two columns, deliberately not merged: the things you wrote down, grouped by overdue,
