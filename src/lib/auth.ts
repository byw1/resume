 ...
 * self-hosted one. The host comes from the Public URL setting where it is set,
 * and from the request otherwise, the same order everything else here uses.
 */
export async function signedInHintDomain(): Promise<string | null> {
  ...
  let host = "";
  try {
    host = publicUrl.trim() ? new URL(publicUrl).host : "";
  } catch {
    host = "";
  }
  if (!host) host = (await headers()).get("host") ?? "";
  return sharedCookieDomain(host, landingUrl);
}
